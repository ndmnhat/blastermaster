...
void CCannonBullet::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x;
	top = y;
	right = left + CANNON_BULLET_BBOX_WIDTH;
	bottom = top - CANNON_BULLET_BBOX_HEIGHT;
}

CCannonBullet::CCannonBullet()
{
	this->Speed = CANNON_BULLET_SPEED;
	this->Damage = CANNON_BULLET_DAMAGE;
	this->LifeTime = CANNON_BULLET_LIFETIME;
	LPANIMATION_SET ani_set = CAnimationSets::GetInstance()->Get(CANNON_BULLET_ANIMATION_SET_ID);
