...
void CBossBullet::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
...
void CBossBullet::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x;
	top = y;
	right = left + BOSS_BULLET_BBOX_WIDTH;
	bottom = top - BOSS_BULLET_BBOX_HEIGHT;
}

CBossBullet::CBossBullet()
{
	this->Speed = BOSS_BULLET_SPEED;
	this->Damage = BOSS_BULLET_DAMAGE;
	this->LifeTime = BOSS_BULLET_LIFETIME;
	LPANIMATION_SET ani_set = CAnimationSets::GetInstance()->Get(BOSS_BULLET_ANIMATION_SET_ID);
