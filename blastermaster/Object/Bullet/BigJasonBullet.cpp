...
void CBigJasonBullet::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
...
void CBigJasonBullet::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x;
	top = y;
	right = left + BIGJASON_BULLET_BBOX_WIDTH;
	bottom = top - BIGJASON_BULLET_BBOX_HEIGHT;
}
